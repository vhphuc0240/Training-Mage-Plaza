...
const ListTodo = ({ todoList, setTodoList, loading }) => {
  ...
  const handleCompleteTodo = async (id) => {
    ...
    setTodoList((prev) =>
      prev.map((todo) =>
        todo.id === id && todo.status !== "success" ? newData : todo,
      ),
    );
    setSelectedTodos([]);
  };
  ...
  const completedPromotedBulkActions = async () => {
    const selectedTodoToUpdate = selectedTodos.map((id) => ({
      id,
      updateField: { status: "success" },
    }));
