...
const isWaiting = window.location.pathname === '/waiting';
const MainLayout = ({children}) => {
  return isEmbeddedApp ? (
    <AppBridgeProvider>
      {isWaiting ? <> {children}</> : <AppLayout>{children}</AppLayout>}
    </AppBridgeProvider>
  ) : (
    <>{isWaiting ? <> {children}</> : <AppFullLayout>{children}</AppFullLayout>}</>
  );
};
